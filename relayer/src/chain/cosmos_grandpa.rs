...
use crate::light_client::cosmos_grandpa::LightClient as COSMOSGPLightClient;
...
pub struct CosmosGrandpaSdkChain {
    config: ChainConfig,
...
impl ChainEndpoint for CosmosGrandpaSdkChain {
    ...
    fn init_light_client(&self) -> Result<Self::LightClient, Error> {
        ...
        //     .map_err(|e| Error::rpc(self.config.rpc_addr.clone(), e))?;
        //
        // let light_client = GpLightClient::from_config(&self.config, peer_id)?;
        let light_client = COSMOSGPLightClient::new();
    ...
    fn query_client_state(
        ...
        let client_state = downcast!(client_state.clone() => AnyClientState::Grandpa)
    ...
    fn query_upgraded_client_state(
        ...
        let tm_client_state = downcast!(client_state => AnyClientState::Grandpa)
            .ok_or_else(|| Error::client_type_mismatch(ClientType::Grandpa, client_type))?;
    ...
    fn query_upgraded_consensus_state(
        ...
        let tm_consensus_state = downcast!(consensus_state => AnyConsensusState::Grandpa)
            .ok_or_else(|| {
                Error::consensus_state_type_mismatch(ClientType::Grandpa, cs_client_type)
            })?;
    ...
    fn proven_client_consensus(
        ...
        let consensus_state =
            downcast!(consensus_state.clone() => AnyConsensusState::Grandpa)
...
async fn broadcast_tx_sync(chain: &CosmosGrandpaSdkChain, data: Vec<u8>) -> Result<Response, Error> {
    let response = chain
        .rpc_client()
...
async fn query_account(chain: &CosmosGrandpaSdkChain, address: String) -> Result<BaseAccount, Error> {
...
async fn do_health_check(chain: &CosmosGrandpaSdkChain) -> Result<(), Error> {
