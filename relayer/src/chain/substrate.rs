    ...
    fn proven_packet(
        ...
        let result = retry_with_index(Fixed::from_millis(200), |current_try| {
            ...
            let result = async {
                ...
                match packet_type {
                    PacketMsgType::Recv => {
                        octopusxt::call_ibc::get_packet_commitment(&port_id, &channel_id, sequence.0, client).await
                    }
                    PacketMsgType::Ack => {
                        octopusxt::call_ibc::get_packet_ack(&port_id, &channel_id, sequence.0, client).await
                    }
                    _ => unimplemented!(),
                }
            };
