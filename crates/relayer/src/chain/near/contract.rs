...
pub async fn test123() -> anyhow::Result<()> {
    let client = NearRpcClient::new("https://near-testnet.infura.io/v3/4f80a04e6eb2437a9ed20cb874e10d55");
    let connection_id = ConnectionId::new(5);
    let connection_id = serde_json::to_string(&connection_id).unwrap();
    dbg!(&connection_id);

    dbg!(&connection_id.eq("connection-5"));
    let result = client.view("nearibc.testnet".parse()?, "get_connection_end".to_string(),
                             json!({"connection_id": "connection-5".to_string()}).to_string().into_bytes()).await.unwrap();
