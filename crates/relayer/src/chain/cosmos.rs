...
impl ChainEndpoint for CosmosSdkChain {
    ...
    fn build_connection_proofs_and_client_state(
        ...
        let sig_data = super::super::foreign_client::alice_sign_sign_bytes(
            sequence + 1,
            timestamp,
            ("/ibc/connections%2F".to_string() + connection_id.as_str()).into(),
    ...
    fn build_channel_proofs(
        ...
        let sig_data = super::super::foreign_client::alice_sign_sign_bytes(
            sequence + 1,
            timestamp,
            ("/ibc/channelEnds%2Fports%2F".to_string()
                + port_id.as_str()
                + &"%2Fchannels%2F".to_string()
                + channel_id.as_str())
            .into(),
        ...
        let (maybe_packet_proof, channel_proof) = match packet_type {
            PacketMsgType::Recv => {
                ...
                let sig_data = super::super::foreign_client::alice_sign_sign_bytes(
                    seq + 1,
                    timestamp,
                    ("/ibc/commitments%2Fports%2F".to_string()
                        + port_id.as_str()
                        + "%2Fchannels%2F"
                        + channel_id.as_str()
                        + "%2Fsequences%2F"
