...
// use ibc_proto::ibc::lightclients::solomachine::v2::DataType;
use ibc_proto::ibc::lightclients::solomachine::v3::HeaderData;
...
impl<DstChain: ChainHandle, SrcChain: ChainHandle> ForeignClient<DstChain, SrcChain> {
    ...
    pub fn build_update_client_with_trusted(
        ...
        let timestamp = header.timestamp().nanoseconds();
        let sig_data =
            alice_sign_sign_bytes(sequence, timestamp, "solomachine:header".to_string().as_bytes().to_vec(), buf.to_vec());
        let th = match header {
            AnyHeader::Tendermint(header) => {
                let th: TmHeader = TmHeader::from(header);
                println!("ys-debug: TmHeader: {:?}", th.timestamp());
...
pub fn alice_sign_sign_bytes(
    ...
    use ibc_proto::ibc::lightclients::solomachine::v3::SignBytes;

    println!(
        "ys-debug: sequence {:?}, timestamp: {:?}, path: {:?}, data: {:?}",
        sequence, timestamp, path, data
    );
    let bytes = SignBytes {
        sequence,
        timestamp,
