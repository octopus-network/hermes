...
use crate::{
    chain::{endpoint::HealthCheck, handle::ChainHandle, tracking::TrackingId, ChainType},
    config::Config,
    event::beefy_monitor,
    event::{
...
fn spawn_beefy_workers<Chain: ChainHandle>(
    ...
    tracing::debug!(
        "substrate::spawn_beefy_workers -> chains_scan ={:?}",
        chains_scan
    );
    ...
    for (src_chain, subscription) in subscriptions {
        tracing::debug!(
            "substrate::spawn_beefy_workers -> src_chain = {:?}",
            src_chain
        );
        tracing::debug!(
            "substrate::spawn_beefy_workers -> subscription = {:?}",
            subscription
        );
        ...
        let handle = spawn_background_task(
            error_span!("worker.beefy", chain = %src_chain.id()),
            // Some(Duration::from_millis(5)),
            Some(Duration::from_secs(1)),
            move || -> Result<Next, TaskError<Infallible>> {
                if let Ok(beefy) = subscription.try_recv() {
                    handle_beefy(
                        &config,
                        &mut registry.write(),
                        &mut client_state_filter.acquire_write(),
...
fn init_beefy_sub<Chain: ChainHandle>(
    ...
    for chain_config in chains {
        ...
        let chain_type = chain_config.r#type.clone();
        match chain_type {
            ChainType::Substrate => {
                let chain = match registry.get_or_spawn(&chain_config.id) {
                    Ok(chain) => chain,
